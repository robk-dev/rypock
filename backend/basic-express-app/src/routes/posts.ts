    ...
    try {
        ...
        if (error.code && error.code === 11000) {
            return res
                ...
                .send({ error: { message: status['409_MESSAGE'], code: status['409_NAME'] } });
        }
        return res
            .status(status['INTERNAL_SERVER_ERROR'])
            .send({ error: { message: status['500_MESSAGE'], code: status['500_NAME'] } });
    }
});
...
router.put('/:id', async (req: express.Request, res: express.Response) => {
    ...
    try {
        ...
        return res
            ...
            .send({ error: { message: status['404_MESSAGE'], code: status['404_NAME'] } });
    }
});
...
router.delete('/:id', async (req: express.Request, res: express.Response) => {
    ...
    try {
        ...
        return res
            .status(status['OK'])
            .send({ error: { message: status['200_MESSAGE'], code: status['200_NAME'], data: result } });
    } catch (error) {
        logger.error('Error updating user', error);
        return res
            .status(status['INTERNAL_SERVER_ERROR'])
