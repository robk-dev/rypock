import { IConnectionManager, IRepo, ILogger } from "../interfaces";
...
export class RedisRepo<T> implements IRepo<T> {
    ...
    public async find(key: string): Promise<any> {
        return new Promise((resolve, reject) => {
            this.cache.getClient().get(key, (error: any, result: string) => {
                if (error) {
    ...
    public async set(key: string, value: string, options: IRedisInsertOptions = {}): Promise<any> {
        return new Promise((resolve, reject) => {
            ...
            this.cache
                .getClient()
                .set(key, value, options.command || "EX", options.expiryTime || 3600, (error, result) => {
                    if (error) {
