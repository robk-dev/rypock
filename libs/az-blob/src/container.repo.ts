...
import { IContainer } from './blob.repo.interface';
import { Blob } from './blob.repo';
...
export class Container implements IContainer {
    ...
    constructor(options: ContainerOptions, private logger: any) {
    ...
    public async listBlobs(nextMarker?: string, options?: IContainerListBlobsSegmentOptions) {
        const listResponse = await this.containerURL.listBlobFlatSegment(Aborter.none, nextMarker, options);
    ...
    public async listAllBlobs() {
        ...
        let blobs: any[] = [];
        do {
            const listResponse = await this.containerURL.listBlobFlatSegment(Aborter.none, nextMarker);
